...
export async function GET() {
  ...
  const res = await fetch(`${WASENDER_API_BASE}/whatsapp-sessions/${sessionId}/qrcode`, {
    headers: { 'Authorization': `Bearer ${pat}` },
  })

  if (!res.ok) {
    const errText = await res.text()
    console.error('[wasender/qrcode] WasenderAPI error:', errText)
    return NextResponse.json({ error: 'QR kod alınamadı' }, { status: 502 })
  }

  // Response: { success: true, data: { qrCode: '2@...' } }
